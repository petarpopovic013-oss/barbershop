      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:gap-10">
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="admin-schedule-card overflow-hidden rounded-[16px] border border-white/10 bg-[#141417] shadow-xl sm:rounded-[20px]">
            ...
            {/* Full-day schedule grid — responsive on all screens */}
            <div className="overflow-x-auto bg-[#141417] -webkit-overflow-scrolling-touch">
              {reservations.length === 0 && !fetchError ? (
                <div className="admin-empty-state flex flex-col items-center justify-center px-4 py-16 text-center sm:px-6 sm:py-24">
                  <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full border-2 border-white/20 bg-white/5 text-white/70 sm:mb-6 sm:h-20 sm:w-20">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="sm:w-8 sm:h-8">
                      <rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
                    </svg>
                  </div>
                  <p className="font-heading text-[16px] text-white/90 sm:text-[18px] md:text-[20px]">NEMA REZERVACIJA</p>
                  <p className="mt-1.5 text-[12px] text-white/50 sm:mt-2 sm:text-[13px]">Izaberite drugi datum ili proverite kasnije</p>
                </div>
              ) : (
                <div className="admin-schedule-grid grid bg-[#141417]"
                  style={{
                    gridTemplateColumns: `48px repeat(${columns}, minmax(80px, 1fr))`,
                    gridTemplateRows: `auto repeat(${slots.length}, ${SLOT_MINUTES * pxPerMin}px)`,
                    minWidth: columns > 2 ? `${48 + columns * 120}px` : undefined,
                  }}>
                  <div className="border-r border-b border-white/10 bg-[#141417] p-1 sm:p-2" style={{ gridColumn: 1, gridRow: 1 }} />
                  {displayBarbers.map((b, colIndex) => (
                    <div key={b.id} className="border-b border-r border-white/10 bg-[#141417] px-1 py-2.5 text-center font-heading text-[10px] uppercase tracking-wider text-white/90 sm:px-4 sm:py-4 sm:text-[13px]"
                      style={{ gridColumn: colIndex + 2, gridRow: 1 }}>{b.name}</div>
                  ))}
                  {displayBarbers.length === 0 && (
                    <div className="border-b border-r border-white/10 bg-[#141417] px-1 py-2.5 text-center text-[11px] text-white/40 sm:px-3 sm:py-4 sm:text-sm" style={{ gridColumn: 2, gridRow: 1 }}>—</div>
                  )}
                  {slots.map((slot, rowIndex) => (
                    <div key={slot} className="border-r border-b border-white/10 bg-[#141417] py-1.5 pr-1 text-right text-[9px] font-medium tracking-wider text-white/50 sm:py-2 sm:pr-3 sm:text-[11px]"
                        ...
                        {colReservations.map((r) => {
                          ...
                          return (
                            <button
                              ...
                              className="admin-reservation-block absolute left-0.5 right-0.5 rounded border border-white/25 bg-white/8 px-1.5 py-1 text-left text-[9px] leading-tight transition-all duration-300 hover:border-white/40 hover:bg-white/15 sm:left-2 sm:right-2 sm:rounded-lg sm:px-4 sm:py-2.5 sm:text-xs focus-ring"
                              style={getBlockStyle(r)}
                            >
                              <span className="block truncate font-semibold text-[#F5F5F7]">{r.customer_name || r.customer_phone || "—"}</span>
                              <span className="hidden truncate text-[#A1A1A6] sm:block">{serviceLabel}</span>
                              <span className="block truncate text-[8px] text-[#6B6B70] sm:text-[10px]">{getLocalTimeString(r.start_time)}–{getLocalTimeString(r.end_time)}</span>
                            </button>
                          );
                        })}
                      </div>
